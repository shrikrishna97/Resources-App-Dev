...
import smtplib 
from model import User, Reservation, ParkingLot, ParkingSpot
...
def send_daily_reminder():
    ...
    for res in active_reservations:
        ...
        send_email(user.email, "Daily Reminder", f"Hello {user.username}, this is your daily reminder for your active reservation with vehicle number {res.vehicle_number}.")
...
@celery_app.task
def export_reservations(user_id):
    user = User.query.get(user_id)
    if not user:
        return "User not found"
    reservations = Reservation.query.filter_by(user_id=user_id).all()
    if not reservations:    
        return "No reservations found for the user"
